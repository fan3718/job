...
export default app.directive('ngResourceChapterTreeview',['$compile',function($compile){
		...
		return {
			...
			template:require('./resource-treeview.htm'),
			compile: function(element){
				return cpl(element, function(scope, iElement, iAttrs, controller, transcludeFn){
					...
					scope.checkboxClick=function(chapter, type, event,count){
						if(typeof scope.ngChapterClick === 'function'){
							scope.ngChapterClick(chapter,type,event.target,count);
						}
					};
