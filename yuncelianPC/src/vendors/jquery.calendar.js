	...
	jQuery.fn.extend({
		...
		calendar : function(options){
			...
			if($(this).is('input')){
				text = $(this).val();
				if(text==''){
					$(this).val(format(today,options.format));
				}
			}else{
				text = $(this).text();
				if(text==''){
			...
			function initDom(container){
				...
				var header = $('<div class="ui-calendar-header"></div>');
				...
				mTitleView = $('<div class="ui-calendar-title"></div>');


				mLargeTitleView = $('<div class="ui-calendar-large-title"></div>');
				...
				mWeekContainer = $('<div class="ui-calendar-week"></div>');
				...
				mCompMonth = $('<span class="comp-month">'+formatNumber((mMonth+1))+'</span>');
				...
				mContent = $('<div class="ui-calendar-content"></div>');
			...
			if(renderId != ''){
				...
				if(container.length==0){
					if(container.length==0){
						container = $('<div class="ui-calendar"></div>');
			...
			function bindToolbarEvent(){
				...
				switch(mode){
					...
					case MODE_YEAR_RANGE:
						...
						var range = calcLargeYearRange(year);
				...
				mCompMonth.text(formatNumber(mMonth+1));
			...
			function bindEvent(){
				...
				mContent.find('span').on('click',function(){
					...
					if('day' == role){
						...
						var value = format(newDate,options.format);
						...
						options.onPicked.call(this,format(newDate,options.format));
			 ...
			 * 计算12年的区间范围
			 */
			function calcYearRange(year,count){
				var result = {};
			 ...
			 *	计算144年的区间范围
			 */
			function calcLargeYearRange(year){
				return calcYearRange(year,12*12);
			}
