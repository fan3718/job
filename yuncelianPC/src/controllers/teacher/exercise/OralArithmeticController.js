...
export default app.controller('OralArithmeticController',['$scope','$window','$location','$timeout','ExerciseService','$filter','Toolkit',function($scope,$window,$location,$timeout,ExerciseService,$filter,Toolkit){
		...
		$scope.tabGrade = function (grade) {
			if ($scope.selGrade.child_id != grade.child_id) {
		...
		$scope.changeSection = function (section,type) {
			if (type == '') {
			...
			if(section.isSelected){
				...
				$scope.selSection[key] = {"id":section.id,'name':section.name,type:type,count:section.count};
				$scope.selAllCount += section.count
			} else{
				if ($scope.selSection[key]){
					if ($scope.selAllCount - section.count >= 0) {
						$scope.selAllCount -= $scope.selSection[key].count;
					}
		 ...
		 */
		 function getInitExerciseName(arr){
			...
			if(ids.length >= 2){
				...
				var chapter = $scope.selSection[ids[0]+'_14']
		...
		function getGrades () {
			ExerciseService.getCalclationGrades().then(function (data){
				if (data.grades && data.grades.length > 0){
					...
					getSections($scope.selGrade);
				}
			});
		};

		function getSections (grade) {
			ExerciseService.getCalclationChapters(grade.child_id).then(function (data){
				data.chapters.forEach(function (item,index){
					item['children'] = item['sections'];
				});
				//data.chapters[0].expandable = true;
				// data.tree[0].expandable = true;
				grade["expend"] = !grade.expend ;
				$scope.gradeTrees[grade.description] = data.chapters;
